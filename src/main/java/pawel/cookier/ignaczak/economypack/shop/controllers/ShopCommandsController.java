...
public class ShopCommandsController implements IShopCommandsController {
    ...
    public void removeCategoryFromShop(Player player, Inventory inventory, String[] args) {
        if (player.isOp()) {
            if (!doesCategoryNotExist(player, inventory, args, PluginConfig.REMOVE_SHOP_CATEGORY_COMMAND)
    ...
    public void editShopCategoryName(Player player, Inventory inventory, String[] args) {
        if (player.isOp()) {
            if (hasCorrectNumberOfArgs(player, args, PluginConfig.EDIT_SHOP_CATEGORY_NAME_COMMAND)
                    && !doesCategoryNotExist(player, inventory, args, PluginConfig.EDIT_SHOP_CATEGORY_NAME_COMMAND)
    ...
    public void editShopCategoryItemStack(Player player, Inventory inventory, String[] args) {
        if(player.isOp()){
            ...
            && !doesCategoryNotExist(player, inventory, args, PluginConfig.EDIT_SHOP_CATEGORY_ICON_COMMAND)){
                String categoryName = args[0];
                Material material = Material.getMaterial(args[1]);
                ItemStack[] contents = inventory.getContents();
    ...
    private boolean isAddShopCategoryValid(Player player, Inventory inventory, String[] args) {
        return hasCorrectNumberOfArgs(player, args, PluginConfig.ADD_SHOP_CATEGORY_COMMAND)
                && areArgsCorrectType(player, args)
                && doesCategoryNotExist(player, inventory, args, PluginConfig.ADD_SHOP_CATEGORY_COMMAND)
    ...
    private boolean hasCorrectNumberOfArgs(Player player, String[] args, String command) {
        ...
        switch (command) {
            ...
            case PluginConfig.EDIT_SHOP_CATEGORY_NAME_COMMAND -> player
                    .sendMessage(ChatColor.RED + "Musisz podać starą i nową nazwę kategorii " +
                            "/%s <stara nazwa> <nowa nazwa>"
                                    .formatted(PluginConfig.EDIT_SHOP_CATEGORY_NAME_COMMAND));
            case PluginConfig.EDIT_SHOP_CATEGORY_ICON_COMMAND -> player
                    .sendMessage(ChatColor.RED + "Musisz podać nazwę kategorii i materiału obiektu " +
    ...
    private boolean doesCategoryNotExist(Player player, Inventory inventory, String[] args, String command) {
        String categoryName = args[0];

        List<String> itemNamesArray = IShopUtility.getItemNamesFromInventory(inventory);

        if (itemNamesArray.contains(categoryName)) {
            if (command.equals(PluginConfig.ADD_SHOP_CATEGORY_COMMAND)) {
        ...
        if (command.equals(PluginConfig.REMOVE_SHOP_CATEGORY_COMMAND)
                || command.equals(PluginConfig.EDIT_SHOP_CATEGORY_NAME_COMMAND)
                || command.equals(PluginConfig.EDIT_SHOP_CATEGORY_ICON_COMMAND)) {
